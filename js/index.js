...
function get_new_phrase(current_exercise){
	switch (current_exercise)
	{	case "number_year_recent":
			current_phrase = gn_year_recent();
		break;
		case "number_year_historical":
			current_phrase = gn_year_historical();
		break;
		case "number_age_human":
			current_phrase = gn_age_human();
		break;
		case "number_money_small":
					current_phrase = gn_money_cafe_restaurant();
		break;
		default:
			current_phrase = "je ne sais pas";
		break;
	}
...
function set_speech_rate(new_speech_rate){
	speech_rate = new_speech_rate;
}
...
/*
 * FUNCTION: changed_excercise_type()
 */
function changed_excercise_type() {
		var new_exercise = "";
...
function speak(text) {
  ...
  // Set the attributes.
	...
	msg.rate = parseFloat((Math.random()*0.2)+speech_rate);// parseFloat(rateInput.value);
...
document.getElementById('speech-msg').onkeydown = function(e){
   if(e.keyCode == 13){
		...
		var lowerCasePhraseEnglish = "" + current_phrase;
		...
		lowerCasePhraseEnglish = removeDiacritics(lowerCasePhraseEnglish);
