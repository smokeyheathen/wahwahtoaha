...
function speak(text) {
  ...
  // Set the attributes.
	...
	msg.rate = parseFloat((Math.random()*0.2)+1);// parseFloat(rateInput.value);
  ...
  // utterance instance's voice attribute.
	/*if (voiceSelect.value) {
		voiceSelect.value = "Google français";
...
document.getElementById('speech-msg').onkeydown = function(e){
   if(e.keyCode == 13){
		...
		var lowerCasePhraseEnglish = frenchPhraseList[randomNumber][1];
		...
		if ((lowerCasePhrase == lowerCaseInput)||(lowerCasePhraseEnglish == lowerCaseInput)) {
			...
			randomNumber = Math.floor((Math.random() * (listLength)));
			speak(frenchPhraseList[randomNumber][0]);
		}
		else
		{			
			speak(frenchPhraseList[randomNumber][0]);
